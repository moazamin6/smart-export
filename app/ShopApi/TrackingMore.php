...
class TrackingMore
{
    ...
    protected $apiKey = 'c98d2a1c-1b0e-45a8-8f06-73b60bda71ee';
    ...
    */
    ...
    public function detectAvailableCarrier($tracking_number)
    {
        $carrierArray = $this->detectCarrier($tracking_number)['data'];
        $carrierCode = '';
        foreach ($carrierArray as $carrier) {
            $trackingData = $this->getSingleTrackingResult($tracking_number, $carrier['code'])['data'];
            if ($trackingData !== []) {
     ...
     * @return array
     */
    public function createTracking($trackingNumber, $extraInfo = array())
    {

        $returnData = array();
        $sendData = array();
        $requestUrl = self::ROUTE_CREATE_TRACKING;
        $carrierArray = $this->detectCarrier($trackingNumber)['data'];
        $deleteAbleCarriers = [];
        $tracking = [];
        $previousTracking = [];
        foreach ($carrierArray as $carrier) {
            ...
            $sendData['tracking_number'] = $trackingNumber;
            $sendData['carrier_code'] = $carrier['code'];
            ...
            $tracking = $this->getSingleTrackingResult($trackingNumber, $carrier['code'])['data'];

            if ($tracking !== []) {
                if ($tracking['status'] !== TM_NOT_FOUND) {

                    if ($previousTracking !== []) {
                        $dateOne = new DateTime($tracking['lastUpdateTime']);
                        $dateTwo = new DateTime($previousTracking['lastUpdateTime']);

                        if ($tracking['lastUpdateTime'] !== '' || $previousTracking['lastUpdateTime'] !== '') {
                            if ($dateOne >= $dateTwo) {
                                $returnData = $tracking;
                                $deleteAbleCarriers = array_merge($deleteAbleCarriers, [$previousTracking['carrier_code']]);
                            } else {
                                $returnData = $previousTracking;
                                $deleteAbleCarriers = array_merge($deleteAbleCarriers, [$tracking['carrier_code']]);
                            }
                        }
                    }

                } else {
                    $deleteAbleCarriers = array_merge($deleteAbleCarriers, [$carrier['code']]);
                }
            }
...
//        foreach ($carrierArray as $carrier) {
//            $this->deleteTrackingItem($carrier['code'], $trackingNumber);
//        }
     ...
     * @return array
     */
    public function getSingleTrackingResult($trackingNumber, $carrierCode = null, $lang = '')
    {
        $returnData = array();
        if ($carrierCode === null) {
            $carrierCode = $this->detectCarrier($trackingNumber)['data'][0]['code'];
        }
